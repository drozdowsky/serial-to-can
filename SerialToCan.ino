...
void setup() {
  ...
  CAN1.begin(CAN_TX, CAN_RX, 9600);  // PINs for CAN + CAN rate

  // setup timer
  Timer1.initialize(100000);  // every 0.1s (100000 microseconds)
  Timer1.attachInterrupt(requestData);

  resetRequestData();  // start
}
...
void sendCanMessage(unsigned char requestedPIDlow) {
  ...
  switch (requestedPIDlow) {
    ...
    // These are only added to handle non OBD2 stuff
...
void handleN() {
  ...
  resetRequestData();  // restart data reading
}
...
void readSerial() {
  ...
  switch (currentCommand) {
    ...
    case 'R':  // Speeduino requests data in R-message
      ...
      break;
    default:
      // Consider reading whole 'corrupted' data here.
#ifdef DEBUG
...
#ifdef DEBUG
  if ((millis() - oldtime) > 120000) {  // DEBUG timeout 120s (120000ms)
    Serial.write("Timeout");
    resetRequestData();  // restart data reading
  }
#else
  if ((millis() - oldtime) > 500) {  // timeout 0.5s (500ms)
    resetRequestData();              // restart data reading
  }
#endif
